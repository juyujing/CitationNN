...
def load_data(model_args):
    ...
    citation_file = directory + "citations.txt"
    src_nodes, dst_nodes = load_citation_data(citation_file)

    # 创建 DGL 异质图
    graph_data = {
        ('paper', 'cites', 'paper'): (src_nodes, dst_nodes),  # (src, dst) 边索引对
        ('paper', 'cited_by', 'paper'): (dst_nodes, src_nodes)  # 反向边
    }
    cite_graph = dgl.heterograph(graph_data)
