...
class DirGCN(nn.Module):
    ...
    def forward(self, h):
        # 计算正向信息传递（论文 A 引用 B）
        cites_msg = self.cites_conv(self.graph['cites'], h)
        
        # 计算反向信息传递（论文 B 被 A 引用）
        cited_by_msg = self.cited_by_conv(self.graph['cited_by'], h)
        
        # 拼接 (原始节点特征 + 正向传播结果 + 反向传播结果)
        hidden_emb = torch.cat([cites_msg, cited_by_msg], dim=1)
        out_emb = (1-self.alpha)*self.fc(hidden_emb)+self.alpha*h
        return out_emb
...
class CitationNN(nn.Module):
    def __init__(self, data_config, args_config, adj_mat, u2u, cite_graph, mode='item'):
        ...
        self.gcn = self._init_layer_1(u2u)
        self.dirgcn = self._init_layer_2()
    ...
    def _init_weight(self):
        ...
        self.cite_graph = self.cite_graph.to(self.device)

    def _init_layer_1(self, u2u):
